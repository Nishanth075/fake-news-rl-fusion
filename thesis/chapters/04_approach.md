...
## 4.2 Hypothesis and Inspiration
...
## 4.3 Inputs and Outputs of the Proposed Extension
...
## 4.4 Process Workflow of the Proposed Extension
...
The architectural novelty is the replacement of a fixed post-hoc fusion rule with a sample-level controller that observes reliability evidence before assigning modality weights. The novelty comparison diagram in this chapter makes this distinction explicit by separating simple fixed fusion from the proposed reliability-aware action-selection path.

## 4.5 Reliability-Aware State Representation
...
## 4.6 Adaptive Fusion Action Space
...
## 4.8 Explanation Strategy

The framework provides explanation at three levels. At the image level, Grad-CAM heatmaps highlight visual regions that contributed to the image prediction. At the text level, token saliency identifies influential tokens from the text branch. At the fusion level, the selected action, image weight, text weight, final probability, and final prediction explain how the modalities were combined.

This explanation strategy is aligned with the research problem because it does not stop at model performance. It also provides evidence about why the final prediction was formed in a particular way.

## 4.9 Positioning within the AI Body of Knowledge
...
The contribution is not the invention of a new image encoder or language model. Instead, the contribution is the use of a reliability-aware reinforcement learning controller to make image-text fusion adaptive and inspectable for fake news detection.

...
\section{Unimodal Branch Results}
...
\section{Baseline and Fusion Results}
...
\section{Policy Analysis and Reliability Behaviour}
...
\section{Seed Stability and Statistical Considerations}
...
The evaluation supports the usefulness of multimodal fusion because fusion methods outperformed both image-only and text-only baselines. The result helps interpret why equal fusion remained competitive: the final Fakeddit split contained two reasonably informative modalities, and the average of their probabilities already captured much of the complementary signal. Therefore, the role of the proposed controller is not best understood as replacing fixed fusion with a consistently higher-scoring classifier, but as exposing when and how modality contribution changes at sample level.


The ablation results support the value of reliability-aware state features because the full state outperformed reduced state settings. The policy analysis further suggests that the controller often selected higher image weights in the main seed because the image branch contributed complementary evidence after text-only predictions had already captured strong semantic signal. Image-quality degradation produced the clearest adaptive behaviour because blur, contrast, and entropy directly affected visual usability. Text-quality adaptation was weaker because length and availability are coarse proxies and do not measure semantic truthfulness or model uncertainty.


The matched MLP result suggests that an unconstrained learned controller over the same state is not automatically better. The discrete RL action space forces decisions to remain within interpretable modality-weight choices, which may improve robustness against overfitting in the fusion layer. At the same time, the small and statistically non-significant difference from strong fixed fusion shows that adaptive fusion should be defended as a transparent and competitive framework rather than as a universally superior classifier.


The OpenFake case study explains the external-transfer limitation. The text branch retained ranking signal after calibration, but default probabilities were poorly calibrated under the new dataset distribution. The image branch and fusion controller transferred weakly because visual style, post format, label construction, and image-text relationships differed from Fakeddit. This supports the need for domain adaptation and calibration before using the framework as a real-world cross-domain detector.



\section{External Generalization Case Study}

...
The improvement over the strongest fixed fusion baseline was modest. Therefore, the result should be interpreted as competitive performance with a small main-run improvement, rather than as a large predictive-dominance claim. The additional contribution of the proposed method is explicit decision transparency: it provides sample-level fusion actions and modality weights, which fixed fusion baselines do not provide.

The matched supervised MLP fusion baseline reached 0.8547 macro F1 at the default threshold and 0.8565 macro F1 after threshold tuning. This fairer comparison used the matched supervised fusion evidence files and shows that the adaptive RL fusion result remained stronger in the final reported run. A likely reason is that the supervised MLP learned a direct discriminative mapping from the reliability state to a label, while the RL controller learned a constrained action over interpretable weight pairs. The action constraint may act as an inductive bias: it limits the controller to transparent fusion behaviours instead of allowing arbitrary nonlinear decision boundaries.
...
## Policy Analysis and Reliability Behaviour
...
The text-quality degradation behaviour was less consistent. In the tested configuration, the model did not always adapt in the expected direction for text-quality drops. A likely explanation is that the implemented text-quality proxy mainly measured usability signals such as length and availability, while the text branch probability and confidence already dominated the fusion state. This limitation is discussed in Chapter 8, and it suggests that text reliability estimation requires stronger validation and uncertainty-aware features.
...
## Seed Stability and Statistical Considerations
